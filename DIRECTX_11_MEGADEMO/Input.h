...
enum KeyCode
{
	...
	KEY_OEM_102         = 0x56,    /* <> or \| on RT 102-key keyboard (
	KEY_F11             = 0x57,
	KEY_F12             = 0x58,
	...
	KEY_PREVTRACK       = 0x90,    /* Previous Track (DIK_CIRCUMFLEX on
...
public:
	...
	friend bool  operator==(const MouseEvent& left, const MouseEvent& right)
	{
		return ((2 * left.code + left.state) == (2 * right.code + right.state)); // (left.code == right.code)*(left.state == right.state);
	};
	friend bool  operator<(const MouseEvent& left, const MouseEvent& right)
...
public:
	...
	/* keyboard */
	std::map<KeyEvent, func_vector> keys;
	
	void AddKeyboardHandler(KeyCode code, KeyState keyEvent, func_void func);
	
	//void Run(unsigned int nMsg, WPARAM wParam, LPARAM lParam);

	/* mouse */
	func_vector_coords left_button_up_funcs;
	func_vector_coords left_button_down_funcs;
...
private:
	...
	KeyState CheckKey(KeyCode state);
